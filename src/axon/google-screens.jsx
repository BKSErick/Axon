...
export function GoogleAdsOverview({ onOpen, onNewCampaign, onFilter }) {
  ...
  return <div className="fadein">
    ...
    <GoogleNote />
    <div className="kpi-row"><KPI label={<><GoogleMark /> Investimento</>} value={totalSpend} fmtVal={fmt.brl} delta={6.4} /><KPI label="Cliques" value={totalClicks} fmtVal={fmt.int} delta={9.8} /><KPI label="Conversões" value={totalConv} fmtVal={fmt.int} delta={14.2} /><KPI label="CPA médio" value={cpa} fmtVal={fmt.brl} delta={-3.1} negative /></div>
    <div className="sp-20" />
    <div className="grid-3" style={{ gridTemplateColumns: '2fr 1fr' }}>
      <div className="card"><div className="card-head"><div><div className="card-title">Investimento diário</div><div className="card-sub">Série de performance Google</div></div></div><div style={{ padding: '12px 8px 8px', height: 280 }}><R.ResponsiveContainer width="100%" height="100%"><R.AreaChart data={MOCK_PERFORMANCE_30.map(d => ({ label: d.label, v: d.spend * 0.7 }))}><defs><linearGradient id="googleSpend" x1="0" x2="0" y1="0" y2="1"><stop offset="0%" stopColor="#4285F4" stopOpacity={0.32}/><stop offset="100%" stopColor="#4285F4" stopOpacity={0}/></linearGradient></defs><R.CartesianGrid stroke="rgb(var(--border-soft))" vertical={false}/><R.XAxis dataKey="label" tickLine={false} axisLine={false}/><R.YAxis tickLine={false} axisLine={false}/><R.Tooltip content={<TT prefix="R$ " />} /><R.Area type="monotone" dataKey="v" stroke="#4285F4" fill="url(#googleSpend)" strokeWidth={1.8}/></R.AreaChart></R.ResponsiveContainer></div></div>
    ...
    <div className="sp-20" />
    <div className="card"><div className="card-head"><div><div className="card-title">Campanhas Google</div><div className="card-sub">Ordenado por investimento</div></div><button className="btn btn-sm btn-ghost"><I.search /></button></div><table className="tbl"><thead><tr><th>Campanha</th><th>Tipo</th><th className="right">Spend</th><th className="right">Cliques</th><th className="right">Conv.</th><th className="right">CTR</th><th className="right">CPC</th><th>Status</th></tr></thead><tbody>{rows.map(c => <tr key={c.id} onClick={() => onOpen?.(c.id)} style={{ cursor: 'pointer' }}><td><div style={{ fontWeight: 600, fontSize: 13 }}>{c.name}</div><div className="mono-id">{c.id}</div></td><td><span className="badge info">{c.type}</span></td><td className="right num">{fmt.brl(c.spend || 0)}</td><td className="right num">{fmt.int(c.clicks || 0)}</td><td className="right num">{c.conv || 0}</td><td className="right num">{c.ctr?.toFixed?.(2).replace('.', ',')}%</td><td className="right num">{fmt.brl(c.cpc || 0)}</td><td><Status s={c.status === 'active' ? 'ok' : 'off'} label={c.status === 'active' ? 'Ativa' : 'Pausada'} /></td></tr>)}</tbody></table></div>
  </div>;
}
...
export function GoogleKeywords({ onNewKeyword, onNewNegative }) {
  ...
  return <div className="fadein"><PageHead eyebrow="Google Ads" title="Palavras-chave" sub="Keywords, negativas e qualidade de busca" actions={<><button className="btn" onClick={onNewNegative}><I.plus />Negativa</button><button className="btn btn-primary" onClick={onNewKeyword}><I.plus />Nova palavra-chave</button></>} /><GoogleNote /><div className="kpi-row" style={{ gridTemplateColumns: 'repeat(3, 1fr)' }}><KPI label="Impressões" value={impr} fmtVal={fmt.int} delta={4.1} /><KPI label="Cliques" value={clicks} fmtVal={fmt.int} delta={7.6} /><KPI label="CTR médio" value={impr ? `${((clicks / impr) * 100).toFixed(2).replace('.', ',')}%` : '0,00%'} delta={1.3} /></div><div className="sp-20" /><div className="card"><div className="card-head"><div><div className="card-title">Palavras-chave ativas</div><div className="card-sub">Termos com entrega no período</div></div></div><table className="tbl"><thead><tr><th>Termo</th><th>Match</th><th className="right">Impr.</th><th className="right">Cliques</th><th className="right">CPC</th><th className="right">Conv.</th><th className="right">QS</th></tr></thead><tbody>{keywords.map((k, i) => <tr key={`${k.kw}-${i}`}><td style={{ fontWeight: 600, fontSize: 13 }}>{k.kw}</td><td><span className="badge">{k.match}</span></td><td className="right num">{fmt.int(k.impr || 0)}</td><td className="right num">{fmt.int(k.clicks || 0)}</td><td className="right num">{fmt.brl(k.cpc || 0)}</td><td className="right num">{k.conv || 0}</td><td className="right num">{k.qs || 0}/10</td></tr>)}</tbody></table></div><div className="sp-20" /><div className="card"><div className="card-head"><div><div className="card-title">Negativas</div><div className="card-sub">Listas aplicadas para limpar tráfego</div></div></div><table className="tbl"><thead><tr><th>Termo</th><th>Lista</th><th className="right">Ações</th></tr></thead><tbody>{negatives.map((n, i) => <tr key={`${n.kw}-${i}`}><td className="txt-mono" style={{ fontSize: 13 }}>{n.kw}</td><td className="muted" style={{ fontSize: 12 }}>{n.list}</td><td className="right"><button className="btn btn-sm btn-ghost btn-icon"><I.trash /></button></td></tr>)}</tbody></table></div></div>;
}

export function ClientGoogleAds({ onNewCampaign }) {
  const { googleCampaigns } = useAxonData();
  const rows = googleCampaigns.slice(0, 8);
  return <div className="fadein"><PageHead eyebrow="Mídia paga" title="Google Ads" sub="Suas campanhas Google" actions={<button className="btn btn-primary" onClick={onNewCampaign}><I.plus />Nova campanha</button>} /><GoogleNote /><div className="card"><div className="card-head"><div><div className="card-title">Campanhas</div><div className="card-sub">Resumo de performance</div></div></div><table className="tbl"><thead><tr><th>Campanha</th><th>Tipo</th><th className="right">Spend</th><th className="right">Conv.</th><th>Status</th></tr></thead><tbody>{rows.map(c => <tr key={c.id}><td style={{ fontWeight: 600, fontSize: 13 }}>{c.name}</td><td><span className="badge info">{c.type}</span></td><td className="right num">{fmt.brl(c.spend || 0)}</td><td className="right num">{c.conv || 0}</td><td><Status s={c.status === 'active' ? 'ok' : 'off'} label={c.status === 'active' ? 'Ativa' : 'Pausada'} /></td></tr>)}</tbody></table></div></div>;
}