...
export function AdminReports({ onSend, onSchedule }) {
  ...
  return <div className="fadein"><div className="page-head"><div><div className="eyebrow">Automação</div><h1 className="page-title">Relatórios</h1><div className="page-sub">Envios automáticos via WhatsApp • PDFs gerados</div></div><div className="page-actions"><button className="btn" onClick={onSchedule}><I.cal />Configurar agenda</button><button className="btn btn-primary" onClick={onSend}><I.send />Disparar todos</button></div></div><div className="kpi-row"><KPI label="Enviados (30d)" value={ok} delta={null} /><KPI label="Na fila" value={queued} delta={null} /><KPI label="Falhas (24h)" value={failed} negative delta={null} /><KPI label="Taxa de sucesso" value={success} delta={null} /></div><div className="sp-20" /><div className="card"><div className="card-head"><div><div className="card-title">Histórico de envios</div><div className="card-sub">Últimos {rows.length} relatórios</div></div><div className="row"><select className="select" style={{ width: 200 }}><option>Todos os clientes</option>{clients.map(c => <option key={c.id}>{c.name}</option>)}</select><div className="seg"><button className="on">Todos</button><button>Falhas</button><button>Fila</button></div></div></div>{loading.reports ? <div className="card-pad"><Skel h={240} /></div> : rows.length === 0 ? <div className="card-pad empty">Nenhum relatório enviado ainda.</div> : <table className="tbl"><thead><tr><th>Relatório</th><th>Cliente</th><th>Destinatário</th><th>Período</th><th>Data</th><th>Status</th><th className="right">Ações</th></tr></thead><tbody>{rows.map((r) => { const c = clients.find(x => x.id === r.client); return <tr key={r.id}><td><div style={{ fontWeight: 500, fontSize: 13 }}>{r.title || 'Relatório'}</div><div className="muted" style={{ fontSize: 11 }}>via WhatsApp</div></td><td>{c ? <div className="row" style={{ gap: 8 }}><span className={`avt ${c.color}`}>{c.logo}</span><span style={{ fontSize: 13 }}>{c.name}</span></div> : <span className="muted">—</span>}</td><td className="num" style={{ fontSize: 12 }}>{r.phone || '—'}</td><td className="muted" style={{ fontSize: 12 }}>{r.period}</td><td className="muted" style={{ fontSize: 12 }}>{r.date ? `${new Date(r.date).toLocaleDateString('pt-BR')} • ${new Date(r.date).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}` : '—'}</td><td>{r.status === 'ok' && <span className="badge success"><I.check className="ico" style={{ width: 11, height: 11 }} />Entregue</span>}{r.status === 'queued' && <span className="badge info"><I.clock className="ico" style={{ width: 11, height: 11 }} />Na fila</span>}{r.status === 'failed' && <span className="badge danger"><I.x className="ico" style={{ width: 11, height: 11 }} />Falhou ({r.attempts || 3} tent.)</span>}{!['ok', 'queued', 'failed'].includes(r.status) && <span className="badge">{r.status || '—'}</span>}</td><td className="right"><button className="btn btn-sm btn-ghost" onClick={() => window.location.reload()}><I.refresh /></button>{r.pdfUrl && <a className="btn btn-sm btn-ghost" href={r.pdfUrl} target="_blank" rel="noreferrer"><I.eye /></a>}</td></tr>; })}</tbody></table>}</div></div>;
}
