...
export function AdminSocial() {
  const { clients, loading } = useAxonData();
  const list = clients.slice(0, 4);
  const totalClients = clients.length || list.length;
  const connectedCount = Math.min(3, totalClients);
  return <div className="fadein"><div className="page-head"><div><div className="eyebrow">Orgânico</div><h1 className="page-title">Social Media</h1><div className="page-sub">Status de conexões Instagram/Facebook dos clientes</div></div><div className="page-actions"><button className="btn" onClick={() => { window.toast && window.toast('Sincronizando Instagram Graph API...', 'info'); setTimeout(() => window.toast && window.toast(`${connectedCount} contas atualizadas`, 'success'), 1200); }}><I.refresh />Sincronizar</button></div></div><div className="kpi-row"><KPI label="Perfis conectados" value={`${connectedCount}/${totalClients || 0}`} delta={null} /><KPI label="Posts publicados (30d)" value={68} delta={12.4} /><KPI label="Alcance orgânico total" value="892K" delta={8.7} /><KPI label="Engajamento médio" value="4,82%" delta={0.3} /></div><div className="sp-20" />{loading.clients ? <div className="grid-2"><Skel h={158} /><Skel h={158} /><Skel h={158} /><Skel h={158} /></div> : list.length === 0 ? <Empty icon={<I.insta />} title="Nenhum cliente encontrado" sub="Cadastre clientes para acompanhar as conexões Instagram/Facebook." /> : <div className="grid-2">{list.map((c, i) => { const isConnected = i < connectedCount; const handle = `@${(c.name || 'cliente').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-z0-9]+/g, '').slice(0, 15)}`; return <div className="card" key={c.id}><div className="card-head"><div className="row" style={{ gap: 10 }}><span className={`avt lg ${c.color}`}>{c.logo}</span><div><div className="card-title">{c.name}</div><div className="card-sub">{isConnected ? handle : 'Não conectado'}</div></div></div>{isConnected ? <Status s="ok" label="Conectado" /> : <Status s="off" label="Pendente" />}</div>{isConnected ? <div className="card-pad"><div className="grid-4" style={{ gridTemplateColumns: 'repeat(4,1fr)', gap: 14 }}><Mini label="Seguidores" value={fmt.int(18420 - i * 2300)} delta={5.4 - i * 1.2} /><Mini label="Posts 30d" value={24 - i * 4} delta={null} /><Mini label="Alcance" value="245K" delta={3.2} /><Mini label="Engaj." value={(4.8 - i * 0.4).toFixed(2).replace('.', ',') + '%'} delta={0.4} /></div></div> : <div className="card-pad empty"><I.insta className="ic" style={{ width: 32, height: 32 }} /><div style={{ marginBottom: 12 }}>Cliente ainda não conectou Instagram/Facebook</div><button className="btn btn-sm">Enviar lembrete</button></div>}</div>; })}</div>}</div>;
}
